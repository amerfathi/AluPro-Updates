...
import { AssemblyPartNode, AssemblyTree } from '../domain/configuration/AssemblyTree.js'
import { generate2DPreviewModelFromAssemblyTree } from '../domain/preview/generate2DPreviewModelFromAssemblyTree.js'
import { adapt2DPreviewModelToLegacyVisualModel } from '../domain/preview/generate2DPreviewModelFromAssemblyTree.js'
import { createProfileGeometryCatalog } from '../domain/preview/profileGeometryCatalog.js'
...
const createPreviewAssemblyTree = ({
  wallWidth,
  wallHeight,
  qtyMultiplier,
  safeLabel,
  ...
  const configuredElement = {
    id: `assembly-preview-${safeLabel}`,
    label: safeLabel,
    systemId: String(frameProfile?.id || ''),
    systemType: frameProfile?.systemType || 'custom',
    widthM: wallWidth,
    heightM: wallHeight,
    quantity: qtyMultiplier,
    sections: modulesPreview.map((module, index) => ({
      id: module.id || `section-${index + 1}`,
      type: module.type === 'fixed' ? 'fixed' : 'sash',
      widthM: module.w,
      heightM: module.h
    }))
  }

  const rootId = `${configuredElement.id}-tree`
  const frameNode = new AssemblyPartNode({
    id: `${rootId}-frame`,
    partType: 'frame',
    label: 'Frame',
    quantity: qtyMultiplier,
    metadata: {
      profileId: String(frameProfile?.id || ''),
      profileName: String(frameProfile?.name || ''),
      profileSystemType: String(frameProfile?.systemType || '')
    }
  })

  const sectionNodes = modulesPreview.map((module, index) =>
    new AssemblyPartNode({
      id: `${rootId}-module-${module.id || index + 1}`,
      partType: module.type === 'fixed' ? 'fixed_panel' : 'sash',
      label: module.label || `Section ${index + 1}`,
      quantity: qtyMultiplier,
      metadata: {
        sectionId: module.id || `section-${index + 1}`,
        sectionType: module.type === 'fixed' ? 'fixed' : 'sash',
        widthM: module.w,
        heightM: module.h,
        xM: module.x,
        yM: module.y,
        ...
        profileId: String(module.profile?.id || ''),
        profileName: String(module.profile?.name || ''),
        profileSystemType: String(module.profile?.systemType || '')
      }
    })
  )
  ...
  const rootNode = new AssemblyPartNode({
    ...
    children: [frameNode, ...sectionNodes]
  })
  ...
  return new AssemblyTree({
    ...
    root: rootNode,
    metadata: {
      source: 'assembly-layout-preview-adapter',
      configurationRules: {
        rows: rowTracks.map((row) => ({ id: row.id, start: row.start, end: row.end, size: row.actual })),
        columns: columnTracks.map((column) => ({
          id: column.id,
          start: column.start,
          end: column.end,
          size: column.actual
...
export const compileAssemblyGrid = ({
  ...
  let previewAssemblyTree = null
  let previewModel = null
  try {
    previewAssemblyTree = createPreviewAssemblyTree({
      wallWidth,
      wallHeight,
      qtyMultiplier,
      safeLabel,
    ...
    const profileGeometryCatalog = createProfileGeometryCatalog([
      frameProfile,
      ...modulesPreview.map((module) => module.profile)
    ])
    previewModel = generate2DPreviewModelFromAssemblyTree({
      assemblyTree: previewAssemblyTree,
      profileGeometryCatalog
    })
  } catch (previewError) {
    warnings.push(
      `تعذر توليد نموذج المعاينة الدوميني مؤقتًا: ${previewError?.message || 'خطأ غير معروف'}`
    )
  }

  const visualModel = adapt2DPreviewModelToLegacyVisualModel(previewModel) || legacyVisualModel
