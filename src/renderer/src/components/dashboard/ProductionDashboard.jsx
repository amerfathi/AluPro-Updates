...
const ProductionDashboard = ({
  ...
  const queuePieces =
    Number(productionOrder?.queue?.pieceQty) ||
    neededPieces.reduce((sum, piece) => sum + (Number(piece.quantity) || 0), 0)
  const queueGlass =
    Number(productionOrder?.queue?.glassQty) ||
    neededGlass.reduce((sum, glass) => sum + (Number(glass.quantity) || 0), 0)
  const queueAccessories =
    Number(productionOrder?.queue?.accessoriesQty) ||
    neededAccessories.reduce((sum, accessory) => sum + (Number(accessory.quantity) || 0), 0)
  ...
  const miniStats = [
    {
      ...
      helper: 'ضمن مركز العقود والتنفيذ',
      icon: FolderOpen,
      tone: 'bg-[#eef1ff] text-[#5568ff]'
    },
    {
          ...
          <div className="mb-6 flex items-center justify-between gap-3">
            <div>
              <h3 className="text-xl font-black text-[var(--alu-text)]">
                آخر العقود والملفات المحفوظة
              </h3>
              <div className="mt-1 text-sm font-bold text-[var(--alu-text-soft)]">
                عقود تحت التشغيل ومكتملة ومقايسات محفوظة للرجوع والمتابعة بدون خلط.
              </div>
            </div>
