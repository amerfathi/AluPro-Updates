...
const AssemblyLivePreview = ({ visualModel, frameProfileName = '' }) => {
  if (!visualModel) {
    return (
  ...
  const scale = frontWidth / Math.max(visualModel.wallWidth || 1, 1)
  const frontHeight = Math.max(260, (visualModel.wallHeight || 1) * scale)
  const depth = 26
  ...
  const projectX = (value) => inner.x + (value / Math.max(visualModel.wallWidth || 1, 1)) * inner.w
  const projectY = (value) => inner.y + (value / Math.max(visualModel.wallHeight || 1, 1)) * inner.h

  const moduleRects = (visualModel.modules || []).map((module) => {
    const px = projectX(module.x)
    const py = projectY(module.y)
    const pw = Math.max(1, (module.w / Math.max(visualModel.wallWidth || 1, 1)) * inner.w)
    const ph = Math.max(1, (module.h / Math.max(visualModel.wallHeight || 1, 1)) * inner.h)
    return {
      ...module,
      px,
      py,
  ...
  const verticalMullions = (visualModel.mullions?.vertical || []).map((segment) => ({
    ...segment,
    px: projectX(segment.x),
    py: projectY(segment.start),
    ph: Math.max(
      1,
      ((segment.end - segment.start) / Math.max(visualModel.wallHeight || 1, 1)) * inner.h
    )
  }))

  const horizontalMullions = (visualModel.mullions?.horizontal || []).map((segment) => ({
    ...segment,
    px: projectX(segment.start),
    py: projectY(segment.y),
    pw: Math.max(
      1,
      ((segment.end - segment.start) / Math.max(visualModel.wallWidth || 1, 1)) * inner.w
    )
  }))

  const structuralPieces = visualModel.structuralPieces || []
      ...
      <div className="mt-5 overflow-hidden rounded-[1.75rem] border border-[rgba(225,230,246,0.95)] bg-[linear-gradient(135deg,#f8fbff_0%,#eef3ff_60%,#ffffff_100%)] p-3 shadow-inner">
        <svg viewBox={`0 0 ${viewBoxWidth} ${viewBoxHeight}`} className="h-auto w-full">
          ...
          <DimensionLine
            ...
            label={formatMeasure(visualModel.wallWidth)}
          />
          <DimensionLine
            x1={outer.x - 34}
            y1={outer.y}
            x2={outer.x - 34}
            y2={outer.y + outer.h}
            label={formatMeasure(visualModel.wallHeight)}
            vertical
          />
        </svg>
      </div>
      ...
      <div className="mt-5 rounded-[1.35rem] bg-[#f8faff] p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="rounded-full bg-white px-3 py-1 text-xs font-black text-[#5568ff]">
            تغطية {Number(visualModel.coveragePercent || 0).toFixed(1)}%
          </div>
        </div>
